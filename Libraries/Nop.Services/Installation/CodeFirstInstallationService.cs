...
namespace Nop.Services.Installation
{
    ...
    public partial class CodeFirstInstallationService : IInstallationService
    {
        ...
        protected virtual void InstallLocaleResources()
        {
            ...
            InstallLocaleResourcesVi();
        }
        protected virtual void InstallLocaleResourcesVi()
        {
            //'English' language
            var language = _languageRepository.Table.Single(l => l.Name == "Tiếng Việt");

            //save resources
            var directoryPath = _fileProvider.MapPath(NopInstallationDefaults.LocalizationResourcesPath);
            var pattern = $"viResources.{NopInstallationDefaults.LocalizationResourcesFileExtension}";
            foreach (var filePath in _fileProvider.EnumerateFiles(directoryPath, pattern))
            {
                var localesXml = _fileProvider.ReadAllText(filePath, Encoding.UTF8);
                var localizationService = EngineContext.Current.Resolve<ILocalizationService>();
